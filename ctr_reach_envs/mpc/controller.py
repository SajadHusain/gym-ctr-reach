...
class NonlinearMPC:
    ...
    def plan(self, joints, measured_tip, goal, *, torsion_scaled=None):
        ...
        if eps and (q[n-1] > -eps+1e-12 or np.any(np.diff(q[:n]) < eps-1e-12)):
        ...
        def is_feasible(x, values):
            v = x.reshape(h, width)
            physical = q+self.caps*v[:, :nq]
            if any(not constraints.is_feasible(node) for node in physical):
                return False
            if eps and (np.any(physical[:, n-1] > -eps+1e-12)
                        or np.any(np.diff(physical[:, :n], axis=1) < eps-1e-12)):
                return False
